...
{
    if (((double)(totItems)/CAPACITIES[mIndex_]) >= resizeAlpha_) {
        resize();
    }
    //ItemType x = p;
    HashItem* toInsert = new HashItem(p);
    HASH_INDEX_T loc = probe(p.first);
    if (loc == (HASH_INDEX_T)-1) {
        delete toInsert;
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    ...
    if(h == npos || table_[h] == nullptr || table_[h]->deleted) return;
